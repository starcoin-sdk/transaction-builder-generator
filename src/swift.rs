...
/// Output transaction builders and decoders in Swift for the given ABIs.
...
pub fn write_source_files(
    ...
    write_script_call_files(install_dir.join("Test.swift"), "script", abis)?;
...
fn write_helper_file(
    ...
    let mut file = std::fs::File::create(dir_path.join("Helpers.swift"))?;
...
fn write_script_call_files(
    mut install_dir: std::path::PathBuf,
    package_name: &str,
    abis: &[ScriptABI],
) -> Result<()> {
    ...
    println!("{:?},{:?}",install_dir,script_registry);

    let mut source = File::create(&install_dir).unwrap();

    swift::CodeGenerator::new(&config)

        .output(&mut source, &script_registry)
        .map_err(|err| std::io::Error::new(std::io::ErrorKind::Other, format!("{}", err)))?;
    Ok(())
}

/// Add minimal escaping for Swiftdoc.
...
/// Shared state for the Swift code generator.
